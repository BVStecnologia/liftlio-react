...
### NUNCA fazer:
1. Criar containers browser com docker run (usar orchestrator API)
2. Reiniciar/remover containers manualmente (perde volumes e sessao)
3. Criar containers manualmente sem orchestrator
4. Mudar portas sem atualizar este arquivo
5. Ignorar erros de health check
6. Esquecer de salvar sessao apos login
...
### 2025-12-31
- [x] **CORRECAO CRITICA: Gestao de Containers via Orchestrator**:
  - NUNCA usar docker run manual para criar containers browser
  - SEMPRE usar o orchestrator API: POST http://localhost:8080/containers
  - O orchestrator gerencia volumes automaticamente (claude-credentials, browser-chrome)
  - Se criar manualmente com docker run, perde os volumes e a sessao Google
  - Container atual: liftlio-browser-117 (portas 10100/16000)
  - Orchestrator: browser-orchestrator (porta 8080)
- [x] Proxy Data Impulse configurado na porta 10000 (sticky) para manter sessao
