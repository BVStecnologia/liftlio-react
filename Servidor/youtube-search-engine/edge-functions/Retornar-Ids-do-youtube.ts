...
serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch("http://173.249.22.2:8000/search", {
      ...
      signal: AbortSignal.timeout(50000), // 50 segundos timeout
    });
    ...
    const result = {
      text: videoIdsString,  // CAMPO PRINCIPAL USADO PELAS FUNÇÕES SQL
      success: true,
      data: data.data || data, // Dados completos para debug
    ...
    return new Response(
      JSON.stringify({
        text: "", // Campo vazio em caso de erro
        success: false,
        error: error.message,
        request_id: requestId,
