-- Função SQL: update_video_id_cache
-- Chama a Edge Function Retornar-Ids-do-youtube para buscar vídeos via YouTube Search Engine v5
-- Atualiza o campo "ID cache videos" no scanner com os IDs retornados
-- Se não encontrar vídeos, insere "NOT" no campo

-- LIMPAR FUNÇÕES DUPLICADAS/ANTIGAS
DROP FUNCTION IF EXISTS update_video_id_cache(bigint);
DROP FUNCTION IF EXISTS update_video_id_cache(integer);
DROP FUNCTION IF EXISTS update_video_id_cache(text);

-- CRIAR FUNÇÃO CORRETA QUE CHAMA A EDGE FUNCTION CERTA
