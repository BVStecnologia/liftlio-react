 ...
 */ async function processBatch(tableConfig, records) {
  const results = {
    success: 0,
    failed: 0,
    errors: []
  ...
  for (const record of records){
    try {
      // 1. Preparar conteúdo
      const { data: content, error: prepareError } = await supabase.rpc(tableConfig.prepareFunction, {
        p_id: record.id
      });
      ...
      const { error: insertError } = await supabase.from('rag_embeddings').upsert({
      ...
      const { error: updateError } = await supabase.from(tableConfig.name).update({
        rag_processed: true,
 ...
 */ async function getUnprocessedRecords(tableConfig, limit) {
  if (tableConfig.needsJoin) {
    ...
    const { data, error } = await supabase.from(tableConfig.name).select(`id, ${tableConfig.projectIdField} as project_id`).eq('rag_processed', false).not(tableConfig.projectIdField, 'is', null).limit(limit);
    return {
      data,
      error
    };
