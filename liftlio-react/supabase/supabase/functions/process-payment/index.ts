...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: card, error: cardError } = await supabase.from('cards').select('*, customer:customers(*)').eq('id', card_id).eq('is_active', true).single();
    console.log('Card query result:', {
      card,
      cardError
    });
    if (cardError || !card) {
