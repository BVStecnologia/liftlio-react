 ...
 */ async function handleCreateCheckoutSession(req) {
  ...
  try {
    const { priceId, successUrl = 'https://seu-site.com/success', cancelUrl = 'https://seu-site.com/cancel', customerId = null, mode = 'payment', lineItems = null } = await req.json();
 ...
 */ async function handleGetPaymentConfig(req) {
  if (req.method !== 'GET') {
    return new Response(JSON.stringify({
      error: 'Method not allowed'
    }), {
...
async function handlePaymentIntentSucceeded(paymentIntent) {
  // Aqui você pode implementar sua lógica para pagamentos bem-sucedidos
  // Por exemplo, atualizar o banco de dados, enviar e-mail de confirmação, etc.
  console.log('PaymentIntent succeeded:', paymentIntent.id);
// Em um caso real, você pode conectar ao Supabase aqui:
// const supabaseClient = createClient(supabaseUrl, supabaseServiceKey)
// await supabaseClient.from('orders').update({ payment_status: 'paid' }).eq('payment_intent_id', paymentIntent.id)
}
async function handlePaymentIntentFailed(paymentIntent) {
  // Lógica para pagamentos falhos
  console.log('PaymentIntent failed:', paymentIntent.id, paymentIntent.last_payment_error?.message);
}
async function handleCheckoutSessionCompleted(session) {
  // Lógica para sessões de checkout concluídas
  console.log('Checkout session completed:', session.id);
  // Opcionalmente, busque mais detalhes do pagamento
