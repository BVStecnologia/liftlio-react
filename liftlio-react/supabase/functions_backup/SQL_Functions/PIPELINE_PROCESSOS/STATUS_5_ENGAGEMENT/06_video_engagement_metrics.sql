-- =============================================
-- Função: video_engagement_metrics
-- Tipo: Analytics (relatório de métricas)
--
-- Descrição:
--   Retorna métricas detalhadas de engagement por vídeo:
--   - Taxa de resposta
--   - Lead score médio
...
--   p_limit INTEGER - Máximo de vídeos (default: 50)
--   p_min_comments INTEGER - Mínimo de comentários monitorados (default: 5)
--
-- Saída:
--   TABLE com 24 colunas de métricas (view_count, like_count, engagement_score, etc.)
--
-- Conexões:
--   → Chamada por: Interface/Dashboard
...
CREATE OR REPLACE FUNCTION public.video_engagement_metrics(p_project_id bigint, p_limit integer DEFAULT 50, p_min_comments integer DEFAULT 5)
 RETURNS TABLE(video_id bigint, video_youtube_id text, video_title text, channel_name text, created_date date, days_since_created integer, view_count bigint, like_count bigint, comment_count bigint, monitored_comments bigint, total_posts bigint, posts_responded bigint, response_rate numeric, avg_lead_score numeric, high_quality_leads bigint, engagement_posts bigint, lead_posts bigint, avg_comment_likes numeric, engagement_rate numeric, posts_per_1k_views numeric, lead_quality_index numeric, posting_timeline jsonb, top_comments jsonb, engagement_score numeric)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
