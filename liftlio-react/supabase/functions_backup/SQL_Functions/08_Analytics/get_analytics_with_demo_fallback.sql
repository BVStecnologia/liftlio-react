...
-- Descrição: Retorna dados de analytics reais ou dados de demonstração se não houver dados
-- Criado: 2025-01-24
-- =============================================
...
AS $function$
  ...
  demo_data AS (
    SELECT
      ...
      END AS browser,
      'BR' AS country,
      -- Distribuir corretamente ao longo dos últimos 7 dias
      ...
      END AS is_organic
    FROM
      -- Gerar dados crescentes: menos no início, mais no final
      (SELECT * FROM generate_series(6, 0, -1) AS n) days,
      LATERAL (
        SELECT generate_series(
          1,
          CASE
            WHEN n = 6 THEN 20  -- 7 dias atrás: 20 visitantes
            WHEN n = 5 THEN 25  -- 6 dias atrás: 25 visitantes
            WHEN n = 4 THEN 30  -- 5 dias atrás: 30 visitantes
            WHEN n = 3 THEN 35  -- 4 dias atrás: 35 visitantes
            WHEN n = 2 THEN 45  -- 3 dias atrás: 45 visitantes
            WHEN n = 1 THEN 60  -- 2 dias atrás: 60 visitantes
            WHEN n = 0 THEN 80  -- Hoje: 80 visitantes
          END
        ) AS s
      ) visitors
  )
