...
-- Descrição: Processa transcrição de vídeos em lote (1 por vez)
-- Atualizado: 2025-01-19 (FIX: UPSERT para evitar duplicate key error)
-- =============================================
...
BEGIN
    ...
    -- SOLUÇÃO PARA REFERÊNCIA CIRCULAR + UPSERT PARA EVITAR DUPLICATAS:
    -- 1. Primeiro insira/atualize na tabela Videos_trancricao sem referenciar Videos
    ...
    DO UPDATE SET
        trancription = EXCLUDED.trancription,
        contem = EXCLUDED.contem,
        created_at = now()
    RETURNING id INTO v_transcription_id;

    RAISE NOTICE 'Transcrição inserida/atualizada com ID: %', v_transcription_id;
