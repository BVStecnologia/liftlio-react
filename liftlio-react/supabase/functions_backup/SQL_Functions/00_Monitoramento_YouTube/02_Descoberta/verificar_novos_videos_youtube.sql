...
-- Descrição: Verifica novos vídeos do YouTube em lotes de canais ativos
-- Criado: [Data original]
-- Atualizado: [Última modificação]
-- Features:
--   - Processamento em lotes (padrão 15 canais)
--   - Anti-spam via can_comment_on_channel
--   - Intervalo mínimo de 30 min entre verificações
--   - Sem truncamento de videos_scanreados/processar
--   - Otimização SQL vs Edge Functions (~92% economia)
-- =============================================
      ...
      LOOP
          BEGIN
              ...
              EXCEPTION WHEN OTHERS THEN
                  RAISE WARNING 'Erro ao verificar canal ID % (channel_id: %): %', canal_record.id,
  canal_record.channel_id, SQLERRM;
                  CONTINUE;
              END;
              ...
              IF array_length(videos_novos_array, 1) > 0 THEN
                  ...
                  BEGIN
                      ...
                      IF (api_result->>'text') != 'NOT' THEN
                          ...
                          RAISE NOTICE 'Canal ID %: % vídeos aprovados pela análise', canal_record.id,
  api_result->>'text';
                      ELSE
