...
-- ✅ Respeita Postagem_dia (OBRIGATÓRIO)
--
-- ═══════════════════════════════════════════════════════════════
-- 🎯 NOVA FEATURE V3:
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
    ...
    -- ═══════════════════════════════════════════════════════════════
    -- PASSO 3: VERIFICAR POSTS JÁ AGENDADOS HOJE (V2)
    -- ═══════════════════════════════════════════════════════════════
    ...
    posts_faltantes := posts_por_dia - posts_agendados_hoje;

    RAISE NOTICE '📊 Status: % agendados hoje, faltam %', posts_agendados_hoje, posts_faltantes;

    IF posts_faltantes <= 0 THEN
        RAISE NOTICE '✅ Meta do dia completa (%)​', posts_por_dia;
        RETURN 0;
    END IF;
    ...
    -- 6A. Array de vídeos já usados HOJE
    SELECT ARRAY_AGG(DISTINCT s."Videos")
    INTO videos_usados_hoje
    FROM "Settings messages posts" s
    WHERE s."Projeto" = projeto_id_param
    AND DATE(s.proxima_postagem AT TIME ZONE 'UTC' AT TIME ZONE fuso_horario_projeto) = data_local;

    IF videos_usados_hoje IS NULL THEN
        videos_usados_hoje := ARRAY[]::bigint[];
    END IF;

    RAISE NOTICE '📹 Vídeos usados hoje: % vídeos', COALESCE(array_length(videos_usados_hoje, 1), 0);
    ...
    FOR i IN 1..posts_faltantes LOOP
        ...
        AND cp.lead = true  -- ← Lead qualificado
        ...
        ORDER BY cp.likes DESC, random()  -- Prioriza mais likes
        LIMIT 1;
    ...
    RAISE NOTICE '📊 Total hoje: %/% posts', posts_agendados_hoje + posts_criados, posts_por_dia;
