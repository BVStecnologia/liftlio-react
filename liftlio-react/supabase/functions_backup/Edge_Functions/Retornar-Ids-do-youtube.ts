 ...
 *
 * CHANGELOG V4 (28/12/2025):
 * - CORRIGIDO: Porta 8000 → 8002 (mapeamento externo do container Docker)
 * - Timeout: 240s (4 minutos - margem segura)
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Chamar servidor Python (PORTA 8002 = mapeamento externo do container Docker)
    console.log(`[${requestId}] Calling Python server at 173.249.22.2:8002/search...`);
    const pythonStartTime = Date.now();

    const response = await fetch("http://173.249.22.2:8002/search", {
      method: "POST",
      headers: {
      ...
      signal: AbortSignal.timeout(240000)  // 4 minutos - margem segura
    });
