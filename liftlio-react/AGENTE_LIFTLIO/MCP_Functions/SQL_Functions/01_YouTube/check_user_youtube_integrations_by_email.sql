...
CREATE OR REPLACE FUNCTION public.check_user_youtube_integrations_by_email(p_user_email text)
 RETURNS TABLE(integration_id bigint, project_id bigint, project_name text, youtube_email text, youtube_channel_id text, youtube_channel_name text, is_active boolean, created_at timestamp without time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
