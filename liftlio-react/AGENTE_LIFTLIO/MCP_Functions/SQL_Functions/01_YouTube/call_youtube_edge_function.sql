...
-- Descrição: Chama Edge Function genérica do YouTube
-- Criado: 2025-01-23
-- =============================================

DROP FUNCTION IF EXISTS public.call_youtube_edge_function(integer, text, text);

CREATE OR REPLACE FUNCTION public.call_youtube_edge_function(project_id integer, video_ids text, parts text DEFAULT 'statistics,snippet,contentDetails'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
AS $function$
DECLARE
