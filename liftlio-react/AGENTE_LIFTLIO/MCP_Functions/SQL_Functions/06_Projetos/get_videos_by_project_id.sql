...
-- Descrição: Retorna vídeos paginados associados a um projeto
-- Criado: 2025-01-24
-- Atualizado: Versão com paginação e métricas completas
-- =============================================

CREATE OR REPLACE FUNCTION public.get_videos_by_project_id(projeto_id bigint, page_number integer DEFAULT 1, page_size integer DEFAULT 10)
 RETURNS TABLE(id bigint, nome_do_video text, descricao text, views bigint, commets bigint, target_audience text, content_category text, relevance_score double precision, video_id_youtube text, total_posts bigint, canal_nome text, total_registros bigint)
 LANGUAGE plpgsql
AS $function$
DECLARE
    v_offset INT;
