-- =============================================
-- Função: regenerate_single_comment_response
-- Descrição: Regenera resposta para um único comentário com opção de tipo
-- Criado: 2025-01-23
-- Parâmetros:
--   p_comment_cp_id: ID do comentário na tabela Comentarios_Principais
--   p_response_type: 'engagement' (sem menção), 'product' (com menção), ou 'auto' (Claude decide)
--   p_custom_instruction: Instruções adicionais opcionais
-- =============================================

DROP FUNCTION IF EXISTS regenerate_single_comment_response(INTEGER, TEXT, TEXT);

CREATE OR REPLACE FUNCTION regenerate_single_comment_response(
    p_comment_cp_id INTEGER,
    p_response_type TEXT DEFAULT 'auto',
    p_custom_instruction TEXT DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
AS $$
