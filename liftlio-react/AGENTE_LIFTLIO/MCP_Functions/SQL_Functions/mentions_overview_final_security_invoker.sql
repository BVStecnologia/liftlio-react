-- View final corrigida em 22/07/2025
-- Correções aplicadas:
-- 1. Priorizar postagens agendadas no DISTINCT ON
-- 2. Usar SECURITY INVOKER (padrão) ao invés de SECURITY DEFINER
-- 3. Respeitar RLS das tabelas subjacentes

DROP VIEW IF EXISTS mentions_overview CASCADE;

CREATE OR REPLACE VIEW mentions_overview AS
...
ORDER BY 
    cp.id_do_comentario,
    -- CORREÇÃO PRINCIPAL: Priorizar postagens pendentes primeiro
    CASE 
...
-- NOTA DE SEGURANÇA:
-- Esta view usa SECURITY INVOKER (padrão), o que significa que:
-- 1. Executa com as permissões do usuário que está consultando
-- 2. Respeita todas as políticas RLS das tabelas subjacentes
-- 3. É a abordagem mais segura recomendada pelo Supabase