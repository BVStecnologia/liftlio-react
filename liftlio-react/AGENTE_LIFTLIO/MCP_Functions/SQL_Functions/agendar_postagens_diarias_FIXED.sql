-- =============================================
-- Função: agendar_postagens_diarias (CORRIGIDA)
-- Data: 2025-01-29
-- Correção: Adiciona verificação para evitar duplicação de agendamentos
-- Garante relação 1:1 entre Mensagens e Settings messages posts
-- =============================================

DROP FUNCTION IF EXISTS agendar_postagens_diarias(bigint);

CREATE OR REPLACE FUNCTION agendar_postagens_diarias(projeto_id_param bigint)
RETURNS INTEGER AS $$
DECLARE
...
COMMENT ON FUNCTION agendar_postagens_diarias IS
'Agenda postagens diárias para mensagens não respondidas.
CORREÇÃO 29/01/2025: Adiciona verificação para evitar duplicação - garante que cada mensagem tenha apenas um agendamento.
Mantém relação 1:1 entre Mensagens e Settings messages posts.';