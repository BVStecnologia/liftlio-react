/**
 * Edge Function: Agente Liftlio v32
 * 
 * Assistente AI do Liftlio com RAG, memória de conversação e busca específica de postagens por data
 * 
 * MELHORIAS v32:
 * - Busca direta de postagens de hoje e ontem
 * - Queries específicas para datas ao invés de depender só do RAG
 * - Melhor formatação de respostas sobre postagens agendadas
 * 
 * JSON de teste:
 *{
  ...
  try {
    ...
    const formattedPosts = data?.map(post => {
      ...
      return {
        ...
        video: post.Videos ? {
          title: post.Videos.titulo,
          url: post.Videos.url_video
        } : null
      };
    }) || [];
