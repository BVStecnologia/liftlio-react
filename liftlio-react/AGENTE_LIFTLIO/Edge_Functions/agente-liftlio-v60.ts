 ...
 * JSON de teste:
 *{
 *  "prompt": "quais as postagens de hoje?",
 *  "context": {
 *    "currentProject": {
 *      "id": "58",
 *      "name": "HW"
...
async function getPostsByDate(projectId: string, date: 'today' | 'yesterday', timezone: string, language: 'pt' | 'en') {
  try {
    ...
    const endOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate() + offset);
    endOfDay.setHours(23, 59, 59, 999);
    
    console.log('=== BUSCA POSTAGENS v60 ===');
    console.log('Data:', date);
    console.log('Início do dia:', startOfDay.toISOString());
    console.log('Fim do dia:', endOfDay.toISOString());
    console.log('Projeto:', projectIdNumber);
    
    // Query corrigida - buscar postagens onde postado OU proxima_postagem estão no dia
    const { data, error } = await supabase
      ...
      .or(`postado.gte.${startOfDay.toISOString()},postado.lte.${endOfDay.toISOString()},proxima_postagem.gte.${startOfDay.toISOString()},proxima_postagem.lte.${endOfDay.toISOString()}`)
      .order('proxima_postagem', { ascending: true })
      .order('postado', { ascending: true });
    ...
    console.log(`Encontradas ${data?.length || 0} postagens para ${date}`);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Log para debug
    console.log('=== AGENTE v60 - CORREÇÃO BUSCA POSTS ===');
    console.log('Prompt:', prompt);
    ...
    if (context?.currentProject?.id) {
      ...
      // v60: Buscar postagens de hoje/ontem se necessário
