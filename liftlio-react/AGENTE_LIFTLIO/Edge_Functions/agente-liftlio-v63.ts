/**
 * Edge Function: Agente Liftlio v63 - CORRIGIDO
 * 
 * Correções principais:
 * - Patterns completos para saudações e ferramentas
 * - RPCs reais funcionando
 * - Melhor detecção de intenções
 * - Respostas concisas e formatadas
 */
...
// Mapeamento CORRIGIDO de intenções para funções RPC
const INTENT_TO_RPC = {
  'daily_status': 'get_complete_project_stats', // Função real que existe
  'list_channels': 'list_all_channels',         // Função real que existe
  'show_tools': null, // Não precisa de RPC, é estático
  'performance': 'channel_performance_analysis', // Função real que existe
  'engagement': 'video_engagement_metrics',      // Função real que existe
  'today_posts': null, // Vamos implementar com query direta
  'scheduled_posts': null // Vamos implementar com query direta
};

// Padrões CORRIGIDOS e EXPANDIDOS
const INTENT_PATTERNS = {
  // NOVO: Saudações sempre mostram status
  ...
  // Patterns existentes
  list_channels: /listar canais|liste os canais|todos os canais|quais canais|mostrar canais|canais monitorados/i,
  performance: /performance|desempenho|análise|métricas|estatísticas/i,
...
// Detecta intenção do usuário - MELHORADO
function detectIntent(prompt: string): { intent: string; confidence: number } {
  ...
  // PRIORIDADE 3: Se pergunta curta com "?", assume daily_status
...
// Formata resposta baseada nos dados - MELHORADO
