...
function formatDateTimeBrazil(dateString: string, language: 'pt' | 'en'): string {
  try {
    ...
    // Configurar opções para o timezone de Brasília
...
/**
 * Busca estatísticas reais do projeto (v24 - Com timezone correto)
 */
...
/**
 * Busca RAG otimizada via RPC backend (v24 - Com timezone)
 */
...
/**
 * Formata contexto RAG (v24 - Com timezone de Brasília)
 */
    ...
    (results as any[]).slice(0, 3).forEach((result: any, index: number) => {
      ...
      if (result.metadata) {
        if (result.metadata.scheduled_for) {
          // CORREÇÃO: Usar formatDateTimeBrazil para timezone correto
...
5. Formate datas e horários de forma amigável (sempre mostrando que é horário de Brasília)
6. NUNCA mencione IDs técnicos nas respostas
7. Quando mostrar datas/horas de mensagens agendadas, SEMPRE indique que é "Horário de Brasília"
...
5. Format dates and times in a friendly way (always showing it's Brazil Time)
6. NEVER mention technical IDs in responses
7. When showing scheduled messages dates/times, ALWAYS indicate it's "Brazil Time"
