...
/**
 * Sistema de Memória Aprimorado - v25
...
/**
 * Busca contexto de conversas anteriores - MELHORADO v25
 * Retorna histórico completo e informações extraídas
 */
async function getConversationContext(userId: string, projectId: string | null, sessionId: string) {
  try {
    ...
    // 2. Buscar últimas conversas do usuário no projeto (contexto adicional)
    const { data: userHistory } = projectId ? await supabase
      .from('agent_conversations')
      .select('message, message_type, created_at, metadata')
      .eq('user_id', userId)
...
/**
 * Formata histórico para o prompt - MELHORADO v25
...
/**
 * Processa contexto de tela - NOVO v25
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Log para debug
    console.log('=== AGENTE v25 MEMÓRIA ROBUSTA ===');
    console.log('Prompt:', prompt);
    ...
    // 3. Adicionar contexto de tela (NOVO)
