// @ts-nocheck
/**
 * Edge Function: agente-liftlio (v16 - Métricas Sincronizadas com Dashboard)
 * 
 * Descrição:
 * Versão com métricas perfeitamente sincronizadas com o que o usuário vê na tela.
 * 
 * Melhorias v16:
 * - Usa channels_count direto do RPC (não calcula canais ativos)
 * - Adiciona métrica de mensagens agendadas (Scheduled)
 * - Métricas contextualizadas por página atual
 * - Nomenclatura padronizada com a interface
 * - Adiciona métricas de monitoring (views, likes, engagement)
 * - Remove qualquer discrepância entre agente e dashboard
 * 
...
/**
 * Busca estatísticas reais do projeto - SINCRONIZADAS COM DASHBOARD
 * Esta é a função CRÍTICA que garante que o agente mostra EXATAMENTE o que está na tela
 */
...
/**
 * Gera resposta usando Claude com contexto completo e métricas sincronizadas
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Buscar e adicionar estatísticas reais SINCRONIZADAS se houver projeto
    ...
    // Adicionar estatísticas reais SINCRONIZADAS COM DASHBOARD
    if (projectStats) {
      if (language === 'pt') {
        contextualPrompt += `\n\n## 📊 Estatísticas REAIS do Dashboard:`;
        contextualPrompt += `\n- Menções (Mentions): ${projectStats.totalMentions}`;
        contextualPrompt += `\n- Alcance (Reach/Canais): ${projectStats.totalChannels}`; // USANDO totalChannels!
        contextualPrompt += `\n- Vídeos (Videos): ${projectStats.totalVideos}`;
        contextualPrompt += `\n- Agendadas (Scheduled): ${projectStats.scheduledMessages}`; // NOVO!
        ...
        if (projectStats.mentionsStats) {
          contextualPrompt += `\n\n### 💬 Métricas de Menções:`;
          contextualPrompt += `\n- Menções respondidas: ${projectStats.mentionsStats.totalReplies}`;
          contextualPrompt += `\n- Respostas pendentes: ${projectStats.mentionsStats.pendingReplies}`;
        ...
        contextualPrompt += `\n- Reach (Channels): ${projectStats.totalChannels}`; // USING totalChannels!
        contextualPrompt += `\n- Videos: ${projectStats.totalVideos}`;
        contextualPrompt += `\n- Scheduled: ${projectStats.scheduledMessages}`; // NEW!
        ...
        if (projectStats.mentionsStats) {
          contextualPrompt += `\n\n### 💬 Mentions Metrics:`;
          contextualPrompt += `\n- Replied mentions: ${projectStats.mentionsStats.totalReplies}`;
          contextualPrompt += `\n- Pending replies: ${projectStats.mentionsStats.pendingReplies}`;
    ...
    // System prompt otimizado com înfase na sincronização de métricas
    const systemPrompt = language === 'pt' 
      ? `Você é o assistente AI do Liftlio, uma plataforma de monitoramento de vídeos e análise de sentimentos. 

🚨 REGRA CRÍTICA: Use SEMPRE os números EXATOS fornecidos no contexto. 
- Reach/Alcance = totalChannels (NÃO calcule canais ativos!)
- Scheduled/Agendadas = scheduledMessages
- Mentions/Menções = totalMentions
- Videos/Vídeos = totalVideos

Seja direto, útil e amigável. Mantenha respostas concisas e focadas. NUNCA mencione IDs técnicos nas suas respostas.${contextualPrompt}`
      : `You are the Liftlio AI assistant, a video monitoring and sentiment analysis platform. 

🚨 CRITICAL RULE: ALWAYS use the EXACT numbers provided in the context. 
- Reach = totalChannels (DO NOT calculate active channels!)
- Scheduled = scheduledMessages
- Mentions = totalMentions
- Videos = totalVideos

Be direct, helpful and friendly. Keep responses concise and focused. NEVER mention technical IDs in your responses.${contextualPrompt}`;
