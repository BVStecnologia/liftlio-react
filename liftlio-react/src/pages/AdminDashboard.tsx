    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [
          ...
          supabase.from('Projeto').select('id, "Project name", browser_container_id, browser_mcp_url, browser_vnc_url, browser_session_status').not('browser_container_id', 'is', null),
          supabase.from('browser_tasks').select('*', { count: 'exact' }).order('created_at', { ascending: false }).range(0, 19),
          supabase.from('browser_tasks').select('project_id').order('project_id'),
          supabase.from('browser_tasks').select('created_by').not('created_by', 'is', null),
          supabase.from('browser_logins').select('*').order('created_at', { ascending: false }),
        ...
        const usersData = usersRes.data as { id: string; email: string; created_at: string; last_sign_in_at: string | null; avatar_url: string | null; full_name: string | null }[] | null;
        const projectsData = projectsRes.data;
        const subsData = subsRes.data;
        const paymentsData = paymentsRes.data;
        const containersData = containersRes.data;
        const { data: tasksData, count: tasksCount } = tasksRes;
        const projectIdsData = projectIdsRes.data;
        const taskUsersData = taskUsersRes.data;
        const loginsData = loginsRes.data;
        const waitlistData = waitlistRes.data;
        const promptsData = promptsRes.data;
        const configData = configRes.data;
        const adminConfigData = adminConfigRes.data;
        const analyticsRawData = analyticsRes.data;
